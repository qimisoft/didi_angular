 ...
 * @class LBS
 */
function BestPay_LBS(){};
BestPay_LBS.prototype = {
    /**
     * 获取LBS信息
     * @method getInfo
     * @param {[Json]} params 自定义参数
     *                        --isUpdated: true 为更新取得的位置信息，false为缓存中的位置信息，默认false
     * @param {[Function]} callback － 成功返回时执行的函数
     * @param {[Function]} fail － 失败返回时执行的函数
     * @return {[Object]} 定位后的位置信息对象
     * @example
     * 	function success(object){
     * 		console.log("成功！结果是：" + object);
     * 	}
     * 	function fail(msg){
     * 		console.log("失败！结果是：" + object);
     * 	}
     * 	LBS.getInfo({isUpdated：false}, success, fail);
     * 	成功回调object说明
     * 	{
     * 	latitude:116.481488,//纬度
     *  longitude:39.990464,//经度
     * 	address:"XX省XX市XX街道XX号", 	格式化地址 
 	 ...
 	 *  } 
     */
    getInfo: function(params, callback, fail) {
...
BestPay_User.prototype  = {
	 ...
	 */
	getProduct : function() {
		return '17721256351';//17721026683  18221098402 '18221098402'
	},
	/**
	 * 获取用户地区码
	 * @method getLocation
    ...
    /**
    	 ...
    	 * @return {[String]}
    	 * @example
    	 * 	var ClientInfo = User.getClientInfo();
    	 */
...
BestPay_App.prototype = {
	...
	getSecurityKey : function() {
		return 'testfdslfjewrnlkwernmfklwerlewnlrewrwe';
	},
	...
	getSessionKey : function() {
		return 'testsessionkey1232130ffsf';
	},
	...
	openSafeKeyBoard:function(len,title,success_callback, fail_callback){
		...
		return success_callback("123456")
...
var BestpayHtml5 = {
	...
	callBackJs : function(result, key) {
        ...
        if(status == this.CALL_STATUS.SUCCESS){
            ...
            if (typeof this.CALLBACK_CANCEL[key] == "function"){
